...
const CoursePlayerPrivate_1 = __importDefault(require("../Player/CoursePlayerPrivate"));
...
const CourseContentMedia = ({ refetchUserProgress, videoList, id, activeVideo, setActiveVideo, user, refetch, totalWatched, totalLimitWatch, }) => {
    ...
    const { updateWatchTime, } = (0, useVideoWatchTime_1.default)(id);
    const isOverWatchTime = totalWatched >= (0, dateFomat_1.converTimeHrToSec)(totalLimitWatch);
    (0, react_1.useEffect)(() => {
    ...
    const [completeUserProgress] = (0, coursesApi_1.useCompleteUserProgressMutation)();
    const [updateUserProgress] = (0, coursesApi_1.useUpdateUserProgressMutation)();
    ...
    const handleCompleteVideo = () => {
        ...
        react_hot_toast_1.toast.success("Video Completed");
    ...
    const remainingTime = (0, dateFomat_1.converTimeHrToSec)(totalLimitWatch) - totalWatched;
    if (!videoList) {
        return <div>Loading...</div>;
    }
    return (<div className="w-full sticky top-0 md:max-w-[1000px]">
      {isOverWatchTime ? <>
          <div className="w-full text-center min-h-[20vh] space-y-5">
            <h1 className="text-2xl">คุณหมดโคต้าในการรับชมแล้ว</h1>
            <p className="text-md">คุณรับชมไปทั้งหมด {(0, dateFomat_1.ConvertSecondToLabel)(responseUpdateWatched?.total_watch_time || 1)}</p>
          </div>
        </>
            :
                <CoursePlayerPrivate_1.default courseId={id} onCompleteVideo={handleCompleteVideo} title={videoList?.[activeVideo]?.title} videoUrl={videoList?.[activeVideo]?.videoUrl} onUpdateTime={(time) => setCurrentTime(time)}/>}
      ...
      <div className="w-full flex items-center justify-between my-3">
        ...
        {remainingTime > 0 && <div>
            <span className="text-sm text-gray-400">สามารถเรียนได้อีก {(0, dateFomat_1.ConvertSecondToLabel)(remainingTime)}</span>
          </div>}
        <div className={`${style_1.styles.button} !w-[unset] text-white  !min-h-[40px] !py-[unset] ${videoList?.length - 1 === activeVideo && "!cursor-no-drop opacity-[.8]"}`} onClick={() => setActiveVideo(videoList && videoList?.length - 1 === activeVideo
            ? activeVideo
            : activeVideo + 1)}>
          Next Lesson
      ...
      {activeBar === 1 && (<div>
          {videoList?.[activeVideo]?.links.map((item, index) => (<div className="mb-5" key={index}>
              <h2 className="800px:text-[20px] 800px:inline-block dark:text-white text-black break-words">
                {item.title && item.title + " :"}
              </h2>
              <a className="text-[#4395c4] 800px:text-[20px] 800px:pl-2 break-words" href={item.url}>
                {item.url}
              </a>
            </div>))}
        </div>)}
      ...
      {activeBar === 2 && (<>
          ...
          <div>
            <CommentReply data={videoList} activeVideo={activeVideo} answer={answer} setAnswer={setAnswer} handleAnswerSubmit={handleAnswerSubmit} user={user} questionId={questionId} setQuestionId={setQuestionId} answerCreationLoading={answerCreationLoading}/>
          </div>
        </>)}
